...
const CandidateProfile = ({ candidate, onViewProfile, navigate }) => (
  <div className="flex flex-col md:flex-row justify-between gap-4 mt-6 p-2">
    <ProfileImage image={candidate.image} name={candidate.name} online={candidate.online} />
    <ProfileDetails
      name={candidate.name}
...
const ProfileImage = ({ image, name, online }) => (
  <div className="relative self-center md:self-start">
    ...
    <span
      className={`absolute w-4 h-4 rounded-full border-2 border-white top-18 left-20 ${
        online ? 'bg-[#6B8E23]' : 'bg-[#828282]'
      }`}
    />
  </div>
...
const ProfileDetails = ({ name, type, jobTitle, location, onViewProfile, navigate }) => (
  <div className="ml-0 md:ml-3 flex-1 space-y-1">
    <div>
      <p className="text-[#6B8E23] text-[13px] font-medium">{name}</p>
      <p className="text-[5px] text-[#ffffff]">{type}</p>
    ...
    <ProfileActions onViewProfile={onViewProfile} navigate={navigate} />
  </div>
);

const ProfileActions = ({ onViewProfile, navigate }) => (
  <div className="space-y-1 mt-1 space-x-6">
    <button
      onClick={onViewProfile}
      className="p-1 w-[100px] text-[5px] rounded-3xl bg-[#556B1F] hover:bg-[#6B8E23] text-white font-medium"
    ...
    <button
      onClick={() => navigate('/interview-invite')}
      className="p-1 w-[100px] text-[5px] rounded-3xl border-2
      border-[#6B8E23] hover:bg-[#6B8E23]/10 text-[#ffffff] font-medium"
    >
      Invite for interview
    </button>
  </div>
